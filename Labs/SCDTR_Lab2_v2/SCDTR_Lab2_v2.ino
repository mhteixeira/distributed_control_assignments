...
pid my_pid {0.01, 0.1, 0.5, 0.01, 0,  10, 5};
float r {10.0};
...
//  Serial.print("Integral:");Serial.print(my_pid.I/10);Serial.println("");
  ...
  float flicker_sum = 
    ...
    (dtc_buffer[NUM_MEASUREMENTS-2] - dtc_buffer[NUM_MEASUREMENTS-3]) >= 0?
    0 : abs(dtc_buffer[NUM_MEASUREMENTS-1] - dtc_buffer[NUM_MEASUREMENTS-2]) + 
  ...
  buffer_pos++;
  if (buffer_pos >= NUM_MEASUREMENTS) buffer_pos = 0;
...
//  Serial.print("Flicker_error:");Serial.print(1000*flicker_error);Serial.println("");
  ...
  if (Serial.available()) {
    ...
    if (input.charAt(0) == 'r') {  // if the first character is 'r'
      r = input.substring(2).toFloat();  // update r with the new value
      Serial.print("Reference set:");Serial.println(r);
    }
    if (input.charAt(0) == 'g') {  // if the first character is 'r'
      Serial.print("Reference:");Serial.println(r);
    }
    if (input.charAt(0) == 'e') {  // if the first character is 'r'
      Serial.print("Energy:");Serial.println(energy);
    }
    if (input.charAt(0) == 'v') {  // if the first character is 'r'
      Serial.print("Visibility:");Serial.println(1000*visibility_error);
    }
    if (input.charAt(0) == 'f') {  // if the first character is 'r'
      Serial.print("Flicker error:");Serial.println(1000*flicker_error);
    }
    if (input.charAt(0) == 'g' && input.charAt(2) == 'b' && input.charAt(4) == 'l') {
      for (int i = 0; i <= NUM_MEASUREMENTS - 1; i++) {
        Serial.print(lux_buffer[i]);Serial.print(", ");
      }
      Serial.println("");
    }
    if (input.charAt(0) == 'g' && input.charAt(2) == 'b' && input.charAt(4) == 'd') {
      for (int i = 0; i <= NUM_MEASUREMENTS - 1; i++) {
        Serial.print(dtc_buffer[i]);Serial.print(", ");
      }
      Serial.println("");
    }
  }
